(library (rapid-gambit-yuni compat bitwise primitives)
         (export
...
(define-primitive arithmetic-shift 'arithmetic-shift)
(define-primitive bitwise-ior 'bitwise-ior)
(define-primitive bitwise-and 'bitwise-and)
(define-primitive bitwise-not 'bitwise-not)
         
;; Chicken have no multibyte refs either
...
;; FIXME: Copied from chibi-scheme version
...
(define (bv-read/s64 bv o)
  (let ((hi (bv-read/s32 bv (+ o 4)))
        (lo (bv-read/u32 bv o)))
    (bitwise-ior ;; Perhaps slower than +. Not a science though.
