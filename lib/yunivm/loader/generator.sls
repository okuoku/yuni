...
(define (yuniloader-generate input-args* lib-mapper program k)
  ...
  (define remap-library
    (if lib-mapper
      (lambda (name) (lib-mapper name))
      (lambda (name) name)))
  ...
  (define (process-sequence! pth libname import-clauses export-clauses sequence)
    ...
    (cond
      ((case (car libname)
         ((r7c-basic r7c-equiv r7c-numeric r7c-exceptions r7c-io r7c-os r7c-xxx)
          #t)
         (else #f)) 
       (set! has-macro-export? #t))
  ...
  (let ((code program))
   (let ((import-clauses (car code))
     ...
     (cond
       ((not (eq? 'import (car import-clauses)))
        (error "Malformed program" program) ))
     (process-sequence! #f
