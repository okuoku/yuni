...
(define (make-coreops-fixnum)
  ...
  (let ((string-pool (make-objpool (* 16 1024)))
    ...
    (let ((string-register (string-pool 'REGISTER))
      ...
      (define (fixnum-string-fill! obj c start end)
        (string-fill! (%fixnum-string obj) 
                      (integer->char (fixnum-char->integer c))
                      (%fixnum start) (%fixnum end)))
      (define (fixnum-string-copy! to at from start end)
        (string-copy! (%fixnum-string to) (%fixnum at)
                      (%fixnum-string from) (%fixnum start) (%fixnum end)))
      ...
      (define (fixnum-bytevector-fill! obj b start end)
        (let ((bv (%fixnum-bytevector obj))
              (starti (%fixnum start))
              (endi (%fixnum end))
              (bi (%fixnum b)))
          (let loop ((idx starti))
           (unless (= idx endi)
             ...
             (loop (+ idx 1))))))
      (define (fixnum-bytevector-copy! to at from start end)
        (bytevector-copy! (%fixnum-bytevector to) (%fixnum at)
                          (%fixnum-bytevector from) 
                          (%fixnum start) (%fixnum end)))
      ...
      (define (fixnum-vector-fill! obj fill start end)
        (let ((offs (fixnum-vector->idx obj))
              (starti (%fixnum start))
              (endi (%fixnum end)))
          (when (< (fixnum-vector-length obj) endi)
            (error "Fill overrun"))
          (let loop ((idx starti))
            (unless (= idx endi)
              (heapset! (+ offs idx 3) fill)
              (loop (+ idx 1))))))
      (define (fixnum-vector-copy! to at from start end)
        ;; FIXME: It does not support overwrapping
        (let* ((starti (%fixnum start))
               (endi (%fixnum end))
               (ati (%fixnum at))
               (len (- endi starti))
               (offsto (+ (fixnum-vector->idx to) 3 ati))
               (offsfrom (+ (fixnum-vector->idx from) 3 start))
               (lento (fixnum-vector-length to))
               (lenfrom (fixnum-vector-length from)))
          (when (or (< endi starti)
                    (< lenfrom end)
                    (< lento (+ ati len)))
            (error "Invalid param"))
          (let loop ((idx 0))
           (unless (= idx len)
             (heapset! (+ offsto idx) (heapref (+ offsfrom idx)))
             (loop (+ idx 1))))))
      ...
      (define (query sym)
        (case sym
          ...
          (($string-fill!)       fixnum-string-fill!)
          (($string-copy!)       fixnum-string-copy!)
          ...
          (($bytevector-fill!)   fixnum-bytevector-fill!)
          (($bytevector-copy!)   fixnum-bytevector-copy!)
          ...
          (($vector-fill!)       fixnum-vector-fill!)
          (($vector-copy!)       fixnum-vector-copy!)
