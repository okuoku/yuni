...
(define %%selfboot-program-args (%%extract-program-args
                                  ...
                                  %%selfboot-mypath))
(define %%selfboot-impl-type 'gambit)
(define %%selfboot-core-libs '((yunifake-util scheme-syntax)
                               (yunifake-util core-syntax)
                               ;; FIXME: Remove this when we complete migration
                               ;; from legacy yunifake
                               (yuni scheme)
                               ))
...
(xload (string-append %%selfboot-yuniroot "/lib-runtime/selfboot/common/common.scm"))
(xload (string-append %%selfboot-yuniroot "/lib-runtime/selfboot/common/run-program.scm"))
