...
(define myenv (new-interaction-environment))
...
(define (xload pth) 
  ...
  (let ((prog (file->sexp-list pth)))
   (for-each (lambda (e) (eval e myenv))
             prog)))
...
(define (launcher yuniroot program-args)
  ...
  (xload (string-append yuniroot "/lib-runtime/selfboot/ironscheme/selfboot-runtime.scm"))
