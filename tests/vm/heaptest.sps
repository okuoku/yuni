  ...
  (syntax-rules ()
    ((_ form)
     (let* ((fakeheap (make-coreops-fake))
            ...
            (fixnumheap (make-coreops-fixnum))
            (fixnumbridge (make-hostbridge fixnumheap))
            (fake-host->target (fakebridge 'TARGET))
            (fake-target->host (fakebridge 'HOST))
            (fixnum-host->target (fixnumbridge 'TARGET))
            (fixnum-target->host (fixnumbridge 'HOST)))
       (let ((fake (fake-target->host (fake-host->target form))))
        (check-equal fake form))
       (let ((fixnum (fixnum-target->host (fixnum-host->target form))))
        ;(write (list form '=> fixnum)) (newline)
        (check-equal fixnum form))))))
