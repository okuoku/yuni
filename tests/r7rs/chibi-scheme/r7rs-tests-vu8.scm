...
;; YUNI: define (test ...) as (test-equal ...)
...
(define-syntax test
  (syntax-rules ()
    ((_ c v)
     (test-equal (sexp->string 'v) c v))))

(define-syntax test-values
  (syntax-rules ()
    ((_ values-form testcode)
     (let ((answer (call-with-values (lambda () values-form) list)))
      (test-equal (sexp->string 'testcode) answer (call-with-values (lambda () testcode) list))))))
